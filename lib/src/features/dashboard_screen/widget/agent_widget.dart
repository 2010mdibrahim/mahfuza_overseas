...
class AgentWidget extends StatelessWidget {
  ...
  const AgentWidget({super.key, required this.splashController});
      ...
      children: [
                    ...
                    children: [
                      Expanded(
                        child: ClipRRect(
                          ...
                          child: CommonMethods
                              .cachedNetworkImage(
                            "${splashController.userInformation.value.candidate?.agentPhoto}",
                            // height: 120.0,
                            width: 100.0,
                          ),
                        ),
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: CustomSimpleText(
                                      text:
                                      "${splashController.userInformation.value.candidate?.address}",
                                      fontSize:
                                      14,
                                      alignment:
                                      TextAlign
                  ...
                  child: Column(
                    ...
                    children: [
                      CustomRow(
                          ...
                          "Account Head Name",
                          value:
                          "${(splashController.userInformation.value.candidate?.accountHeadName?.isEmpty ?? false) ? "--" : splashController.userInformation.value.candidate?.accountHeadName}"),
                      ...
                      CustomRow(
                          ...
                          "Email",
                          value:
                          "${(splashController.userInformation.value.candidate?.email?.isEmpty ?? false) ? "--" : splashController.userInformation.value.candidate?.email}"),

                      5.ph,
                      splashController.isBalanceLoading.value == true ? const CircularProgressIndicator() : CustomRow(
                          title:
                          "Balance: ",
                          value:
                          "${splashController.balanceModel.value.data?.balance?.isEmpty ?? false ? "--" : splashController.balanceModel.value.data?.balance}"),
                      5.ph,
                      // CustomRow(
                      //     title:
                      //     "Process job",
        ...
        InkWell(
          ...
          child: Container(
            ...
            height: 40,
            child: Center(child: CustomSimpleText(text: "আমার যাত্রীবৃন্দ", alignment: TextAlign.center, color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold,)),
          ),
        )
      ],
    ));
